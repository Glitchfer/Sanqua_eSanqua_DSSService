...
async function master_Detail(req, res) {
	var joResult;
	var errors = null;
	...
	if (oAuthResult.status_code == '00') {
		if (oAuthResult.token_data.status_code == '00') {
			...
			if (errors.length != 0) {
				...
				joResult = await _masterServiceInstance.getById(req.params);
				// joResult.token_data = oAuthResult.token_data;
				joResult = JSON.stringify(joResult);
			}
		} else {
	...
	if (oAuthResult.status_code == '00') {
		if (oAuthResult.token_data.status_code == '00') {
			...
			if (errors.length != 0) {
				...
				req.query.model = req.params.model;
				req.query.app = req.params.app;
				req.query.logged_employee_id = oAuthResult.token_data.result_verify.employee_info.id;
