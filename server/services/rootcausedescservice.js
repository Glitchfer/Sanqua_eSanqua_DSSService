...
// Repository
const Repository = require("../repository/rootcausedescrepo.js");
const _repoInstance = new Repository();
const RootProblemRepo = require("../repository/rootproblemrepository.js");
const _rootProblemRepo = new RootProblemRepo();
...
const _xClassName = "RootCauseDescService";

class RootCauseDescService {
  async save(pParam) {
    var xJoResult;
    var xAct = pParam.act;
    delete pParam.act;
    ...
    try {
      ...
      if (xAct == "add") {
        if (pParam.hasOwnProperty("logged_user_id") && pParam.hasOwnProperty("rootproblem_id")) {
          if (pParam.logged_user_id != "" && pParam.rootproblem_id != "") {
            xDecId = await _utilInstance.decrypt(
              pParam.rootproblem_id,
              config.cryptoKey.hashKey
            );
            if (xDecId.status_code == "00") {
              pParam.rootproblem_id = xDecId.decrypted;
            ...
            xJoResult = {
              status_code: "-99",
              status_msg: "Invalid logged user or root cause report Id",
            };
          }
          
          if (xFlagProcess) {
            let xDetail = await _rootProblemRepo.getById({id: pParam.rootproblem_id});
        ...
        if (xFlagProcess) {
          ...
          if (xGetDataById.status_code == "00") {
            if (xGetDataById.data.rootproblem.status == 0) {
    ...
    try {
      ...
      if (xLevel) {
        ...
        if (xFlagProcess) {
          ...
          if (xDetail.status_code == "00") {
            if (xDetail.data.rootproblem.status == 0) {
...
module.exports = RootCauseDescService;
