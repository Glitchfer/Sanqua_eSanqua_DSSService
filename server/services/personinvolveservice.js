...
// Repository
const Repository = require("../repository/personinvolverepo.js");
...
const _xClassName = "PersonInvolveService";

class PersonInvolveService {
  async save(pParam) {
    var xJoResult;
    var xAct = pParam.act;
    delete pParam.act;
      ...
      if (xAct == "add" || xAct == "add_batch") {
        if (pParam.hasOwnProperty("logged_user_id") && pParam.hasOwnProperty("initialreport_id")) {
          if (pParam.logged_user_id != "" && pParam.initialreport_id != "") {
            ...
            xJoResult = {
              status_code: "-99",
              status_msg: "Invalid logged user or initial report",
            };
          }
          ...
          xJoResult = {
            status_code: "-99",
            status_msg: "Invalid logged user or initial report id",
          };
        }
      } else if (xAct == "update") {
        if (
    ...
    try {
      ...
      if (xLevel) {
        ...
        if (xFlagProcess) {
          ...
          if (xDetail.status_code == "00") {
            if (xDetail.data.initial_report.status == 0) {
...
module.exports = PersonInvolveService;
