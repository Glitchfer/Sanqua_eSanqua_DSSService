...
class GlobalUtility {
	...
	async generateRoomBookingNo(pId, pRoomId) {
		// Formula : RMB/1/20210101/001
		var dt = dateTime.create();
		var xDate = dt.format('Ymd');
		var xBookNo = 'RMB/' + pRoomId + '/' + xDate + '/' + pId.padStart(5, '0');
		return xBookNo;
	}

	async generateLoanDocumentNo(pId, pDocId) {
		// Formula : RMB/1/20210101/001
		var dt = dateTime.create();
		var xDate = dt.format('ymd');
		var xBookNo = 'LND/' + pDocId + '/' + xDate + '/' + pId.padStart(5, '0');
		return xBookNo;
	}

	async generateReturnDocumentNo(pId, pDocId) {
		// Formula : RMB/1/20210101/001
		var dt = dateTime.create();
		var xDate = dt.format('ymd');
		var xBookNo = 'RTD/' + pDocId + '/' + xDate + '/' + pId.padStart(5, '0');
		return xBookNo;
	}

	async generateArchiveNo(pId, pDocId) {
		// Formula : DOC-001
		var dt = dateTime.create();
		var xDate = dt.format('ymd');
		var xBookNo = 'DOC' + '-' + pId.padStart(3, '0');
		return xBookNo;
	}

	async generateLeaveNo(pId) {
		// Formula : LVE/20210101/00001
		var dt = dateTime.create();
		var xDate = dt.format('ymd');
		var xNo = 'LVE/' + pId + '/' + xDate + '/' + pId.padStart(5, '0');
		return xNo;
	}

	async generatePermissionNo(pId) {
		// Formula : PRM/20210101/00001
		var dt = dateTime.create();
		var xDate = dt.format('ymd');
		var xNo = 'PRM/' + pId + '/' + xDate + '/' + pId.padStart(5, '0');
		return xNo;
	}

	async generateOvertimeNo(pId) {
		// Formula : OVT/20210101/00001
		var dt = dateTime.create();
		var xDate = dt.format('ymd');
		var xNo = 'OVT/' + pId + '/' + xDate + '/' + pId.padStart(5, '0');
		return xNo;
	}
}
